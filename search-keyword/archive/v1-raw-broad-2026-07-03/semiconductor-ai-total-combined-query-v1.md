# V1 - 반도체 제조 AI 특허 - 전체 총합 검색식 풀버전

아래 검색식은 `semiconductor-ai-broad-patent-search.md`의 1~7번 핵심 블록을 압축하지 않고 실제 AND/OR 구조에 치환한 풀버전이다.

구조는 `(반도체 분류 OR 반도체 공통 텍스트) AND 반도체 제조 문맥 AND (AI 분류 OR AI 공통 OR AI 기능/응용 OR AI 알고리즘)`이다. 기존 넓은 검색식의 단어를 유지하기 위해 각 블록을 그대로 결합했다.
...
- 이 파일은 요약본이 아니라 풀버전이다. 기존 블록형 문서의 1~7번 핵심 검색어를 실제 총합 검색식으로 치환했다.
- `%` 절단기호는 검색 DB에 따라 `*`로 바꿔야 할 수 있다.
- `.TI,AB,CL.` 및 `.IPCM,IPC,CPCM,CPC.` 필드 문법도 DB별로 조정해야 한다.
- 1차 검색에서는 `NOT` 제외어를 넣지 않는 편이 안전하다.
- 결과가 너무 많으면 `AI`, `ML`, `VM`, `IC`, `CD`, `smart`, `automation`, `automated` 같은 짧거나 일반적인 단독어부터 줄인다.
- LLM/생성형 AI는 별도 보조 검색식을 한 번 더 돌리는 것이 좋다.
